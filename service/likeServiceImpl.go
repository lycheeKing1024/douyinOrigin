...
func (like LikeServiceImpl) IsFavourite(videoId int64, userId int64) (bool, error) {
	...
	if n, err := redis.RdbLikeUserId.Exists(redis.Ctx, strUserId).Result(); n > 0 {
		...
		if err != nil {
			fmt.Printf("方法:IsFavourite RedisLikeUserId query key失败：%v\n", err)
			return false, err
		}
		exist, err1 := redis.RdbLikeUserId.SIsMember(redis.Ctx, strUserId, videoId).Result()
		//如果有问题，说明查询redis失败,返回默认false,返回错误信息
		if err1 != nil {
			fmt.Printf("方法:IsFavourite RedisLikeUserId query value失败：%v\n", err1)
			return false, err1
		}
		fmt.Printf("方法:IsFavourite RedisLikeUserId query value成功")
		return exist, nil
		...
		if n, err := redis.RdbLikeVideoId.Exists(redis.Ctx, strVideoId).Result(); n > 0 {
			//如果有问题，说明查询redis失败,返回默认false,返回错误信息
			if err != nil {
				fmt.Printf("方法:IsFavourite RedisLikeVideoId query key失败：%v\n", err)
				return false, err
			}
			exist, err1 := redis.RdbLikeVideoId.SIsMember(redis.Ctx, strVideoId, userId).Result()
			//如果有问题，说明查询redis失败,返回默认false,返回错误信息
			if err1 != nil {
				fmt.Printf("方法:IsFavourite RedisLikeVideoId query value失败：%v\n", err1)
				return false, err1
			}
			fmt.Printf("方法:IsFavourite RedisLikeVideoId query value成功")
			return exist, nil
		} else {
			...
			if _, err := redis.RdbLikeUserId.SAdd(redis.Ctx, strUserId, config.DefaultRedisValue).Result(); err != nil {
				fmt.Printf("方法:IsFavourite RedisLikeUserId add value失败")
			...
			if err != nil {
				fmt.Printf("方法:IsFavourite RedisLikeUserId 设置有效期失败")
				redis.RdbLikeUserId.Del(redis.Ctx, strUserId)
				return false, err
			}
...
func (like LikeServiceImpl) FavouriteCount(videoId int64) (int64, error) {
	...
	if n, err := redis.RdbLikeVideoId.Exists(redis.Ctx, strVideoId).Result(); n > 0 {
		//如果有问题，说明查询redis失败,返回默认false,返回错误信息
		if err != nil {
			fmt.Printf("方法:FavouriteCount RedisLikeVideoId query key失败：%v\n", err)
			return 0, err
		}
		//获取集合中userId个数
		...
		if err1 != nil {
			fmt.Printf("方法:FavouriteCount RedisLikeVideoId query count 失败：%v\n", err1)
			return 0, err1
		}
		fmt.Printf("方法:FavouriteCount RedisLikeVideoId query count 成功")
		...
		if _, err := redis.RdbLikeVideoId.SAdd(redis.Ctx, strVideoId, config.DefaultRedisValue).Result(); err != nil {
			fmt.Printf("方法:FavouriteCount RedisLikeVideoId add value失败")
		...
		_, err := redis.RdbLikeVideoId.Expire(redis.Ctx, strVideoId,
			time.Duration(config.OneMonth)*time.Second).Result()
		if err != nil {
			fmt.Printf("方法:FavouriteCount RedisLikeVideoId 设置有效期失败")
			redis.RdbLikeVideoId.Del(redis.Ctx, strVideoId)
			return 0, err
		}
...
func (like LikeServiceImpl) FavouriteAction(userId int64, videoId int64, actionType int32) error {
	...
	if actionType == config.LikeAction {
		...
		if n, err := redis.RdbLikeUserId.Exists(redis.Ctx, strUserId).Result(); n > 0 {
			...
			if err != nil {
				fmt.Printf("方法:FavouriteAction RedisLikeUserId query key失败：%v\n", err)
				return err
			...
			if _, err1 := redis.RdbLikeUserId.SAdd(redis.Ctx, strUserId, videoId).Result(); err1 != nil {
				fmt.Printf("方法:FavouriteAction RedisLikeUserId add value失败：%v\n", err1)
				return err1
			} else {
			...
			if _, err := redis.RdbLikeUserId.SAdd(redis.Ctx, strUserId, config.DefaultRedisValue).Result(); err != nil {
				fmt.Printf("方法:FavouriteAction RedisLikeUserId add value失败")
				redis.RdbLikeUserId.Del(redis.Ctx, strUserId)
				return err
			}
			//给键值设置有效期，类似于gc机制
			_, err := redis.RdbLikeUserId.Expire(redis.Ctx, strUserId,
				time.Duration(config.OneMonth)*time.Second).Result()
			if err != nil {
				fmt.Printf("方法:FavouriteAction RedisLikeUserId 设置有效期失败")
				redis.RdbLikeUserId.Del(redis.Ctx, strUserId)
				return err
			}
			videoIdList, err1 := dao.GetLikeVideoIdList(userId)
		...
		if n, err := redis.RdbLikeVideoId.Exists(redis.Ctx, strVideoId).Result(); n > 0 {
			//如果有问题，说明查询redis失败,返回错误信息
			if err != nil {
				fmt.Printf("方法:FavouriteAction RedisLikeVideoId query key失败：%v\n", err)
				return err
			...
			if _, err1 := redis.RdbLikeVideoId.SAdd(redis.Ctx, strVideoId, userId).Result(); err1 != nil {
				fmt.Printf("方法:FavouriteAction RedisLikeVideoId add value失败：%v\n", err1)
				return err1
			}
			...
			//key:strVideoId，加入value:DefaultRedisValue,过期才会删，防止删最后一个数据的时候数据库还没更新完出现脏读，或者数据库操作失败造成的脏读
			if _, err := redis.RdbLikeVideoId.SAdd(redis.Ctx, strVideoId, config.DefaultRedisValue).Result(); err != nil {
				fmt.Printf("方法:FavouriteAction RedisLikeVideoId add value失败")
				redis.RdbLikeVideoId.Del(redis.Ctx, strVideoId)
				return err
			}
			//给键值设置有效期，类似于gc机制
			_, err := redis.RdbLikeVideoId.Expire(redis.Ctx, strVideoId,
				time.Duration(config.OneMonth)*time.Second).Result()
			if err != nil {
				fmt.Printf("方法:FavouriteAction RedisLikeVideoId 设置有效期失败")
				redis.RdbLikeVideoId.Del(redis.Ctx, strVideoId)
				return err
			}
			userIdList, err1 := dao.GetLikeUserIdList(videoId)
		...
		if n, err := redis.RdbLikeUserId.Exists(redis.Ctx, strUserId).Result(); n > 0 {
			...
			if _, err1 := redis.RdbLikeUserId.SRem(redis.Ctx, strUserId, videoId).Result(); err1 != nil {
				fmt.Printf("方法:FavouriteAction RedisLikeUserId del value失败：%v\n", err1)
				return err1
			} else {
		...
		if n, err := redis.RdbLikeVideoId.Exists(redis.Ctx, strVideoId).Result(); n > 0 {
			...
			if _, err1 := redis.RdbLikeVideoId.SRem(redis.Ctx, strVideoId, userId).Result(); err1 != nil {
				fmt.Printf("方法:FavouriteAction RedisLikeVideoId del value失败：%v\n", err1)
				return err1
			}
...
func (like LikeServiceImpl) GetFavouriteList(userId int64, curId int64) ([]Video, error) {
	...
	if n, err := redis.RdbLikeUserId.Exists(redis.Ctx, strUserId).Result(); n > 0 {
		...
		if err != nil {
			fmt.Printf("方法:GetFavouriteList RedisLikeVideoId query key失败：%v\n", err)
			return nil, err
		}
		//获取集合中全部videoId
		...
		//如果有问题，说明查询redis失败,返回默认nil,返回错误信息
		if err1 != nil {
			fmt.Printf("方法:GetFavouriteList RedisLikeVideoId get values失败：%v\n", err1)
			return nil, err1
		}
		//提前开辟点赞列表空间
		...
		//key:strUserId，加入value:DefaultRedisValue,过期才会删，防止删最后一个数据的时候数据库还没更新完出现脏读，或者数据库操作失败造成的脏读
		if _, err := redis.RdbLikeUserId.SAdd(redis.Ctx, strUserId, config.DefaultRedisValue).Result(); err != nil {
			fmt.Printf("方法:GetFavouriteList RedisLikeUserId add value失败")
			redis.RdbLikeUserId.Del(redis.Ctx, strUserId)
			return nil, err
		}
		//给键值设置有效期，类似于gc机制
		_, err := redis.RdbLikeUserId.Expire(redis.Ctx, strUserId,
			time.Duration(config.OneMonth)*time.Second).Result()
		if err != nil {
			fmt.Printf("方法:GetFavouriteList RedisLikeUserId 设置有效期失败")
			redis.RdbLikeUserId.Del(redis.Ctx, strUserId)
			return nil, err
		}
		videoIdList, err1 := dao.GetLikeVideoIdList(userId)
...
func (like LikeServiceImpl) addFavouriteVideoList(videoId int64, curId int64, favoriteVideoList *[]Video, wg *sync.WaitGroup) {
	...
	video, err := VideoServiceImpl{}.GetVideo(videoId, curId)
	//如果没有获取这个video_id的视频，视频可能被删除了,打印异常,并且不加入此视频
	if err != nil {
		fmt.Println(errors.New("this favourite video is miss"))
		return
	}
	//将Video类型对象添加到集合中去
...
func (like LikeServiceImpl) TotalFavourite(userId int64) (int64, error) {
	//根据userId获取这个用户的发布视频列表信息,返回视频id数组

	videoIdList, err := VideoServiceImpl{}.GetVideoIdList(userId)
	if err != nil {
...
func (like LikeServiceImpl) FavouriteVideoCount(userId int64) (int64, error) {
	...
	if n, err := redis.RdbLikeUserId.Exists(redis.Ctx, strUserId).Result(); n > 0 {
		...
		if err != nil {
			fmt.Printf("方法:FavouriteVideoCount RdbLikeUserId query key失败：%v\n", err)
			return 0, err
		} else {
			count, err1 := redis.RdbLikeUserId.SCard(redis.Ctx, strUserId).Result()
			//如果有问题，说明操作redis失败,返回默认0,返回错误信息
			if err1 != nil {
				fmt.Printf("方法:FavouriteVideoCount RdbLikeUserId query count 失败：%v\n", err1)
				return 0, err1
			}
			fmt.Printf("方法:FavouriteVideoCount RdbLikeUserId query count 成功")
			return count - 1, nil //去掉DefaultRedisValue
		...
		//key:strUserId，加入value:DefaultRedisValue,过期才会删，防止删最后一个数据的时候数据库还没更新完出现脏读，或者数据库操作失败造成的脏读
		if _, err := redis.RdbLikeUserId.SAdd(redis.Ctx, strUserId, config.DefaultRedisValue).Result(); err != nil {
			fmt.Printf("方法:FavouriteVideoCount RedisLikeUserId add value失败")
			redis.RdbLikeUserId.Del(redis.Ctx, strUserId)
			return 0, err
		}
		//给键值设置有效期，类似于gc机制
		_, err := redis.RdbLikeUserId.Expire(redis.Ctx, strUserId,
			time.Duration(config.OneMonth)*time.Second).Result()
		if err != nil {
			fmt.Printf("方法:FavouriteVideoCount RedisLikeUserId 设置有效期失败")
			redis.RdbLikeUserId.Del(redis.Ctx, strUserId)
			return 0, err
		}
		videoIdList, err1 := dao.GetLikeVideoIdList(userId)
...
//	//likeService.VideoServiceImpl = &videoService

...
func Count(videoId int64) (int64, error) {
	...
	var count int64
	//数据库中查询评论数量
	err := SqlSession.Model(Comment{}).Where(map[string]interface{}{"video_id": videoId, "cancel": config.VideoNum}).Count(&count).Error
	if err != nil {
...
func CommentIdList(videoId int64) ([]string, error) {
	...
	err := SqlSession.Model(Comment{}).Select("id").Where("video_id = ?", videoId).Find(&commentIdList).Error
	if err != nil {
...
func InsertComment(comment Comment) (Comment, error) {
	...
	//数据库中插入一条评论信息
	err := SqlSession.Model(Comment{}).Create(&comment).Error
	if err != nil {
...
func DeleteComment(id int64) error {
	...
	//先查询是否有此评论
	result := SqlSession.Model(Comment{}).Where(map[string]interface{}{"id": id, "cancel": config.VideoNum}).First(&commentInfo)
	...
	//数据库中删除评论-更新评论状态为-1
	err := SqlSession.Model(Comment{}).Where("id = ?", id).Update("cancel", config.InvalidComment).Error
	if err != nil {
...
func GetCommentList(videoId int64) ([]Comment, error) {
	...
	result := SqlSession.Model(Comment{}).Where(map[string]interface{}{"video_id": videoId, "cancel": config.ValidComment}).

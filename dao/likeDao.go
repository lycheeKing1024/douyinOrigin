...
func GetLikeUserIdList(videoId int64) ([]int64, error) {
	var likeUserIdList []int64 //保存所有该视频点赞用户id；
	//查询likes表对应视频id点赞用户，返回查询结果
	err := SqlSession.Model(Like{}).Where(map[string]interface{}{"video_id": videoId, "cancel": config.IsLike}).
...
func UpdateLike(userId int64, videoId int64, actionType int32) error {
	...
	err := SqlSession.Model(Like{}).Where(map[string]interface{}{"user_id": userId, "video_id": videoId}).
...
func InsertLike(likeData Like) error {
	...
	err := SqlSession.Model(Like{}).Create(&likeData).Error
	//如果有错误结果，返回插入失败
	if err != nil {
...
func GetLikeVideoIdList(userId int64) ([]int64, error) {
	...
	err := SqlSession.Model(Like{}).Where(map[string]interface{}{"user_id": userId, "cancel": config.IsLike}).
		Pluck("video_id", &likeVideoIdList).Error
	if err != nil {
		//查询数据为0，返回空likeVideoIdList切片，以及返回无错误
		if "record not found" == err.Error() {
			fmt.Println("there are no likeVideoId")
